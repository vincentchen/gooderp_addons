...
class report_custom(report_int):
    ...
    def create(self, cr, uid, ids, datas, context=None):
            ...
            for i in range(4):
                ...
                if field_child:
                    ...
                    if f['fc' + str(i) + '_operande']:
                        ...
                        cond.append((fct[fct_name], f['fc' + str(i) + '_operande'][1], cond_op,
                                     f['fc' + str(i) + '_condition']))
                    else:
                        cond.append(None)
        ...
        fct = {
            ...
            'calc_avg': lambda l: reduce(lambda x, y: float(x) + float(y), filter(None, l), 0) / (
                    len(filter(None, l)) or 1.0),
            ...
            for idx in range(len(fields) - 1):
                ...
                for k in keys:
                    ...
                    if fields[idx + 1]['cumulate']:
                        prev += data[k]
                idx0 = 0
                plot = line_plot.T(label=fields[idx + 1]['name'] + ' ' + str(line), data=data_cum,
    ...
    def _create_bars(self, cr, uid, ids, report, fields, results, context):
        ...
        for line in data_by_year.keys():
            ...
            for idx in range(len(fields) - 1):
                ...
                idx0 = 0
                plot = bar_plot.T(label=fields[idx + 1]['name'] + ' ' + str(line), data=data_cum,
